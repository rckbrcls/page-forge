...
import type { Sha256Digest, VerifiedDescriptorId } from "../../src/domain/models/epub-document";
...
describe("unsafe inspection", () => {
  it("accounts for every later rule without reading content, repairing, or creating output", async () => {
    ...
    expect(result.report.ruleResults.find(({ ruleId }) => ruleId === "finding:ARCHIVE_PATH_TRAVERSAL")).toMatchObject({
      outcome: { status: "completed" },
      findingIds: [terminalFinding.identity],
    });

    const laterResults = result.report.ruleResults.filter(({ ruleId }) => ruleId !== "finding:ARCHIVE_PATH_TRAVERSAL");
    expect(laterResults).toHaveLength(FINDING_CODES.length - 1);
    expect(
      laterResults.every(
        ({ outcome }) => outcome.status === "not_run_after_terminal_finding" && outcome.reason.length > 0,
      ),
    ).toBe(true);
    ...
    expect(source).toEqual(selectedEpub("/fixtures/traversal.epub", "traversal.epub", "unsafe-traversal"));
  });
});
