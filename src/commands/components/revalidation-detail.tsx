...
export function revalidationMarkdown({ comparison, outputName, failure }: RevalidationDetailProps): string {
  const heading = comparison.successful ? "Revalidation Passed" : "Revalidation Failed";
  const introduced =
    comparison.introduced.length === 0
      ? "None"
      : comparison.introduced.map((finding) => `- \`${finding.code}\`: ${finding.title}`).join("\n");

  return [
    `# ${heading}`,
    "",
    outputName ? `Output: \`${outputName}\`` : "The reconstructed copy was compared with the original report.",
    ...
    <List.Item.Detail
      ...
      metadata={
        <List.Item.Detail.Metadata>
          <List.Item.Detail.Metadata.Label title="Revalidation" text={comparison.successful ? "Passed" : "Failed"} />
          <List.Item.Detail.Metadata.Label title="Final Health" text={comparison.finalHealth} />
          <List.Item.Detail.Metadata.Label title="Resolved" text={String(comparison.resolved.length)} />
          <List.Item.Detail.Metadata.Label title="Remaining" text={String(comparison.remaining.length)} />
          <List.Item.Detail.Metadata.Label title="Introduced" text={String(comparison.introduced.length)} />
          {outputName ? <List.Item.Detail.Metadata.Label title="Output" text={outputName} /> : null}
